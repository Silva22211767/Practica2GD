[![Open in MATLAB Online](https://www.mathworks.com/images/responsive/global/open-in-matlab-online.svg)](https://matlab.mathworks.com/open/github/v1?repo=Silva22211767/Practica2GD)
...
2. Analizar la positividad y estabilidad local del sistema de tres EDOs no lineales que modela la interacción entre poblaciones de células patológicas (x), sanas (y) y efectoras (z).
3. Determinar analíticamente los puntos de equilibrio libres de células patológicas y validar la estabilidad asintótica local mediante el cálculo de la matriz Jacobiana y sus respectivos valores propios  ($\lambda$).
...
6. Simular y caracterizar el atractor caótico del sistema utilizando un paso de integración dt = 1*10^-5 y condiciones iniciales específicas para observar la sensibilidad a los parámetros.
